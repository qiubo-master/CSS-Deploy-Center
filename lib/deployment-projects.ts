...
export const builtInProjects: DeploymentProject[] = [
  { id: "css", name: "智能客服", repository: process.env.CSS_GITHUB_REPOSITORY ?? "qiubo-master/CSS", workflow: process.env.CSS_GITHUB_WORKFLOW_FILE ?? "deploy.yml", branch: "master", description: "现有智能客服生产服务", healthUrl: process.env.CSS_HEALTH_URL, endpoint: process.env.CSS_PUBLIC_URL ?? "等待配置访问地址", resourceManaged: false, targetIds: ["aliyun-main"] },
  { id: "media", name: "序章自媒体中台", repository: process.env.MEDIA_GITHUB_REPOSITORY ?? "qiubo-master/Media", workflow: process.env.MEDIA_GITHUB_WORKFLOW_FILE ?? "deploy.yml", branch: "master", description: "内容生产、账号矩阵与 AI 决策中台", healthUrl: process.env.MEDIA_HEALTH_URL, endpoint: process.env.MEDIA_PUBLIC_URL ?? "http://共享公网IP:8080", resourceManaged: true, targetIds: ["aliyun-main"] },
  ...
  { id: "gfm", name: "GFM 通用大模型基座", repository: process.env.GFM_GITHUB_REPOSITORY ?? "qiubo-master/GFM", workflow: process.env.GFM_GITHUB_WORKFLOW_FILE ?? "deploy.yml", branch: "master", description: "统一提供 Qwen 文本、Embedding、YOLO、OCR 与 Qwen-VL 多模态 API", healthUrl: process.env.GFM_HEALTH_URL ?? "https://uu800904-ada3-4b719cf9.westb.seetacloud.com:8443/foundation/v1/health", endpoint: process.env.GFM_PUBLIC_URL ?? "https://uu800904-ada3-4b719cf9.westb.seetacloud.com:8443/", resourceManaged: false, targetIds: ["autodl2"] },
  { id: "deploy-center", name: "CI/CD 发布控制中心", repository: process.env.DEPLOY_CENTER_GITHUB_REPOSITORY ?? "qiubo-master/CSS-Deploy-Center", workflow: process.env.DEPLOY_CENTER_GITHUB_WORKFLOW_FILE ?? "deploy.yml", branch: "master", description: "本控制台自身，支持自举发布", endpoint: process.env.DEPLOY_CENTER_PUBLIC_URL ?? "http://47.113.191.114", resourceManaged: false, targetIds: ["aliyun-main"] },
];
