...
export const builtInProjects: DeploymentProject[] = [
  ...
  { id: "otel", name: "Otel 可观测平台", repository: process.env.OTEL_GITHUB_REPOSITORY ?? "qiubo-master/Otel", workflow: "deploy-otel.yml", branch: "master", description: "统一采集指标、链路与日志，提供 Grafana、Prometheus、Tempo 和 Elasticsearch 可观测能力", projectUrl: process.env.OTEL_PUBLIC_URL ?? "http://100.103.132.88:8103", manualUrl: "https://github.com/qiubo-master/Otel/blob/master/docs/OPERATIONS.md", healthUrl: process.env.OTEL_HEALTH_URL ?? "http://100.103.132.88:8103/api/health", endpoint: process.env.OTEL_PUBLIC_URL ?? "http://100.103.132.88:8103", defaultPort: 8103, centralDeployment: true, resourceManaged: true, targetIds: ["aliyun-main"] },
