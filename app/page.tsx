...
const seed: Dashboard = {
  ...
  projects: [
    ...
    { id: "gfm", name: "GFM 通用大模型基座", repository: "qiubo-master/GFM", branch: "master", description: "统一提供文本、Embedding、视觉检测、OCR 与多模态 API", endpoint: "尚未配置线上访问地址", resourceManaged: false, targetIds: [] },
    { id: "otel", name: "Otel 可观测平台", repository: "qiubo-master/Otel", branch: "master", description: "统一采集指标、链路与日志，提供 Grafana、Prometheus、Tempo 和 Elasticsearch 可观测能力", projectUrl: "http://100.103.132.88:8103", manualUrl: "https://github.com/qiubo-master/Otel/blob/master/docs/OPERATIONS.md", endpoint: "http://100.103.132.88:8103", resourceManaged: true, targetIds: ["aliyun-main"] },
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      <div className="content">
        ...
        <section className="projectStrip" aria-label="部署项目">{data.projects.map((project) => { const projectUrl = project.projectUrl ?? (project.endpoint.startsWith("http") ? project.endpoint : undefined); return <button key={project.id} className={projectId === project.id ? "projectCard selected" : "projectCard"} onClick={() => selectProject(project.id)}><span>{project.id === "media" ? "ME" : "AI"}</span><div><b>{project.name}</b><small>{projectUrl ? <a href={projectUrl} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>访问项目 ↗</a> : "暂未上线"}</small><small><a href={`https://github.com/${project.repository}`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>{project.repository}</a></small></div><em>{projectId === project.id ? "当前项目" : "查看详情"}</em></button>; })}</section>

        <section className="releaseHero" id="release">
          <div className="releaseInfo"><div className="eyebrow"><span className="pulse"/> VERSION DELIVERY</div><h2>{data.project.name}</h2><p>{data.project.description}</p><div className="projectLinks">{(data.project.projectUrl ?? (data.project.endpoint.startsWith("http") ? data.project.endpoint : undefined)) ? <a href={data.project.projectUrl ?? data.project.endpoint} target="_blank" rel="noreferrer">访问项目 ↗</a> : <span>项目暂未上线</span>}<a href={data.project.manualUrl ?? `https://github.com/${data.project.repository}/blob/${data.project.branch}/README.md`} target="_blank" rel="noreferrer">操作手册 ↗</a></div><div className="versionFlow"><div><small>线上版本</small><code>{data.version.deployed ?? data.service.version}</code></div><span>→</span><div><small>GitHub {data.project.branch}</small><code>{data.version.latest?.sha ?? "等待同步"}</code></div>{data.version.updateAvailable && <b className="updateTag">有新版本</b>}</div>{data.version.latest && <p className="commitMessage">{data.version.latest.message} · {data.version.latest.author}</p>}</div>
